...
It is not for contributing to the repository source.
...
**Do NOT use this skill for repository development tasks** (editing the source tree or running `omarchy dev ...` workflows).

## Critical Safety Rules

When invoking a privileged command directly from a graphical context, use
`pkexec` so the desktop can show an authorization prompt with command context. Do
not wrap commands that already manage privilege elevation themselves.

**For end-user customization tasks, NEVER modify anything in `/usr/local/share/rice/`** - but READING is safe and encouraged.

On an installed system this path contains the personal desktop runtime snapshot.
Any direct changes will be:

- Lost on the next installer rerun
- Outside your user configuration and source checkout
- Bypass the source and user-configuration layers

```
/usr/local/share/rice/ # READ-ONLY - NEVER EDIT (reading is OK)
├── bin/                    # Source scripts (symlinked to PATH)
├── config/                 # Default config templates
├── themes/                 # Stock themes
├── default/                # System defaults
├── shell/                  # Desktop shell source and defaults
└── install/                # Installation scripts
```

**Reading `/usr/local/share/rice/` is SAFE and useful** - do it freely to:
...
- Reference default hyprland settings: `cat /usr/local/share/rice/default/hypr/*`
...
If the request is to develop the repository itself, this skill is out of scope. Follow repository development instructions instead of this skill.

## Repository Sync Rule

When a task changes the configured personal desktop repository, finish the
repository workflow automatically after validating the change:

1. Inspect the worktree and preserve unrelated user changes.
2. Stage only files belonging to the task.
3. Create an atomic commit with a succinct message.
4. Push the current branch to its configured upstream.

This is standing authorization for ordinary commits and pushes made to finish
an explicitly requested repository change. It does not authorize force pushes,
history rewrites, remote creation, repository renames, or staging unrelated
files. If commit or push is unavailable, preserve the validated worktree or
local commit and report the blocker.

## Privilege Escalation

For an interactive script or command run in a visible terminal, use `sudo` for
privileged work. The desktop may grant passwordless `sudo` access to particular
...
The personal desktop is built on:

| Component | Purpose | Config Location |
|-----------|---------|-----------------|
| **Arch Linux** | Base OS | `/etc/`, `~/.config/` |
| **Hyprland** | Wayland compositor/WM | `~/.config/hypr/` |
| **Desktop shell** | Status bar + notifications (Quickshell) | `~/.config/omarchy/shell.json` |
| **Launcher** | Quickshell launcher | `~/.config/omarchy/shell.json` |
| **Alacritty** | Default terminal | `~/.config/alacritty/` |
| **Desktop OSD** | On-screen display | Quickshell plugin |

## Command Discovery

The desktop retains a single `omarchy` CLI that dispatches to all `omarchy-*`
...
The repository configures Hyprland in Lua. User files are loaded after its
defaults, so overrides go here:

```
~/.config/hypr/
├── hyprland.lua       # Main config (loads repository defaults, then user files)
...
### Desktop shell (Status Bar + Notifications)
...
# - Desktop shell: shell.json hot-reloads; use `omarchy-shell shell rescanPlugins` for plugin/widget code changes
...
2. See how an existing theme is done via /usr/local/share/rice/themes/catppuccin.
...
Prefer the retained `o.window(match, rules)` helper — see examples in
...
1. **Is it a retained `omarchy` command?** Use it directly
2. **Is it a config edit?** Edit in `~/.config/`, never `/usr/local/share/rice/`
...
5. **Is it a normal package install?** Use `yay -S <pkgs...>` so official and
   ...
   maintaining an inherited script that needs its package abstraction.
...
This skill intentionally does not cover repository source development. Do not use this skill for:
- Editing files in `/usr/local/share/rice/` (`bin/`, `config/`, `default/`, `shell/`, `themes/`, etc.)
