...
panel_id=1
plugin_id=24
printf -v command_path 'env RICE_PATH=%q %q' \
  "$RICE_PATH" "$RICE_PATH/bin/omarchy-xfce-nightlight"
...
xfconf-query -c "$panel_channel" -p "/plugins/plugin-$plugin_id" -n -t string -s genmon 2>/dev/null ||
  xfconf-query -c "$panel_channel" -p "/plugins/plugin-$plugin_id" -s genmon
