...
sed \
  -e "s#/etc/rice.conf#$tmpdir/rice.conf#g" \
  -e "s#/etc/rice-dev.conf#$tmpdir/rice-dev.conf#g" \
  -e "s#/etc/omarchy.conf#$tmpdir/omarchy.conf#g" \
  "$ROOT/default/bash/env-bootstrap" >"$bootstrap"

printf 'export RICE_PATH="/usr/local/share/rice"\n' >"$tmpdir/rice.conf"
mapfile -t default_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
default_path=${default_result[2]}

[[ ${default_result[0]} == /usr/local/share/rice ]] || fail "env-bootstrap resolves installed RICE_PATH" "actual: ${default_result[0]}"
[[ ${default_result[1]} == /usr/local/share/rice ]] || fail "env-bootstrap aliases OMARCHY_PATH to RICE_PATH" "actual: ${default_result[1]}"
pass "env-bootstrap resolves installed RICE runtime"
assert_path_present "$default_path" "$tmpdir/unrelated/bin" "env-bootstrap preserves PATH entries in default mode"

printf 'export RICE_PATH="%s"\nexport OMARCHY_PATH="$RICE_PATH"\n' "$tmpdir/active" >"$tmpdir/rice-dev.conf"
mapfile -t linked_result < <(run_bootstrap bash "$bootstrap" "$home" "$tmpdir/unrelated/bin:/usr/bin")
linked_path=${linked_result[2]}

[[ ${linked_result[0]} == "$tmpdir/active" ]] || fail "env-bootstrap resolves dev-linked RICE_PATH" "actual: ${linked_result[0]}"
