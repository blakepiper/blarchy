...
grep -q 'BLARCHY has pending migrations' "$test_tmp/pending.err" || fail "migration notifier explains pending migrations without notification system"
...
grep -Fx 'Pending BLARCHY Migrations' "$test_tmp/notify-args" >/dev/null || fail "migration notifier uses pending migrations title"
...
wait_for_notify_args &&
  grep -Fx 'Pending BLARCHY Migrations' "$test_tmp/notify-args" >/dev/null ||
